...
| MQTT 5.0 特性：共享订阅 / 用户属性                       | ⚠️ 协议支持 | EMQX Broker 层面已支持 MQTT 5.0 共享订阅（$share/{group}/topic）和用户属性，后端 HiveMQ Client 具备协议层支持，但未显式调用高级特性接口                                                                                                                                                                                                               |
| 主题订阅/取消订阅（精确、+单层、#多层通配）                       | ✅ 已实现   | [MqttTopicUtil.java](backend/src/main/java/com/cde/util/MqttTopicUtil.java) 实现通配符匹配                                                                                                                                                                                                                            |
| 消息生命周期管理（持久化、TTL、死信队列）                        | ⚠️ 部分实现 | EMQX 原生不支持 DLQ：EMQX 是 MQTT Broker，不是消息队列（如 RabbitMQ/Kafka）。消息过期或投递失败时，EMQX 只会丢弃，不会自动转移。                                                                                                                                                                                                                        |
