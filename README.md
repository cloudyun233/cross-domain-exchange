...
| **X.509 证书认证**    | 当前仅支持用户名/密码+JWT 认证方式，未实现 X.509 客户端证书双向认证                                             |
| **主题前缀强制校验**      | 消息发布接口未强制校验主题必须以 `/cross_domain/` 前缀开头，需补充服务端校验逻辑                                     |
| **死信队列（DLQ）**       | 消息过期或多次投递失败后未转入死信队列，EMQX 仅进行自动清理                                                     |
| **审计日志导出**         | 审计日志支持分页查询与条件过滤，但未实现 CSV/PDF 格式导出功能                                                  |
